...
class VariableCheckEvaluator(BaseConditionEvaluator):
    ...
    def evaluate(cls, params: dict, context: Any) -> bool:
        ...
        if operator in ('is_true', 'true'):
            return bool(var_val) is True
        if operator in ('is_false', 'false'):
            return bool(var_val) is False
        ...
        if operator == 'eq':
            return str_var == str_target
        if operator == 'ne':
            return str_var != str_target
        if operator == 'contains':
            return str_target in str_var
        if operator == 'not_contains':
            return str_target not in str_var
        if operator == 'starts_with':
            return str_var.startswith(str_target)
        if operator == 'ends_with':
            return str_var.endswith(str_target)

        # 6. 列表/字典比较
        if isinstance(var_val, list):
            if operator == 'contains':
                return target_val in var_val
            if operator == 'len_eq':
                return len(var_val) == int(target_val)
        if isinstance(var_val, dict) and operator == 'has_key':
            return str(target_val) in var_val
