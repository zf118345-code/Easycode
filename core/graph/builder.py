...
class AdjacencyGraph:
    ...
    def __init__(self):
        ...
        self.adjacency: Dict[str, List[Tuple[str, Optional[str], dict]]] = defaultdict(list)
    ...
    def add_edge(self, source_node: str, target_node: str,
                 target_task: Optional[str] = None, edge_data: dict = None):
        """添加有向边"""
    ...
    def get_out_edges(self, node_id: str) -> List[Tuple[str, Optional[str], dict]]:
        """获取节点的所有出边"""
        return self.adjacency.get(node_id, [])

    def get_target_index(self, node_id: str) -> Optional[int]:
    ...
    def build_from_project(project) -> Dict[str, AdjacencyGraph]:
        ...
        for task_id, task in project.tasks.items():
            ...
            for node in task.nodes:
                if node.on_success and node.on_success.target_node:
                    if node.on_success.target_node not in [
                        t[0] for t in graph.get_out_edges(node.node_id)
                    ]:
                        graph.add_edge(
                            node.node_id,
                ...
                if node.on_failure and node.on_failure.target_node:
                    if node.on_failure.target_node not in [
                        t[0] for t in graph.get_out_edges(node.node_id)
                    ]:
                        graph.add_edge(
                            node.node_id,
    ...
    def build_topology_graph(topology_map) -> AdjacencyGraph:
        ...
        for edge in topology_map.edges:
            graph.add_edge(
                ...
                edge.target_page,
                None,
                {"action": edge.action, "conditions": edge.conditions}
            )
