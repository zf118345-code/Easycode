...
class PathResult:
    ...
    def __init__(self, success: bool, path: list[str] = None, edges: list[dict] = None, reason: str = ''):
    ...
    def to_dict(self) -> dict:
        return {'success': self.success, 'path': self.path, 'edges': self.edges, 'reason': self.reason}
...
class PathFinder:
    ...
    @staticmethod
    def find_shortest_path(graph: AdjacencyGraph, source: str, target: str, avoid_nodes: set[str] = None) -> PathResult:
        """
        BFS 最短路径算法
        :param graph: 邻接表
        ...
        while queue and depth < PathFinder.MAX_SEARCH_DEPTH:
            ...
            for _ in range(level_size):
                ...
                for target_node, _target_task, edge_data in graph.get_out_edges(current):
                    ...
                    if target_node == target:
                        return PathResult(
                            True, path=new_path, edges=new_edges, reason=f'找到最短路径，共 {len(new_path)} 步'
                        )
    ...
    @staticmethod
    def find_path_with_conditions(
        graph: AdjacencyGraph, source: str, target: str, condition_checker=None, avoid_nodes: set[str] = None
    ) -> PathResult:
        """
        ...
        while queue and depth < PathFinder.MAX_SEARCH_DEPTH:
            ...
            for _ in range(level_size):
                ...
                for target_node, _target_task, edge_data in graph.get_out_edges(current):
                    ...
                    if target_node == target:
                        return PathResult(
                            True, path=new_path, edges=new_edges, reason=f'找到条件路径，共 {len(new_path)} 步'
                        )
    ...
    @staticmethod
    def detect_cycle(graph: AdjacencyGraph) -> list[str] | None:
        """
        环路检测：使用 DFS 三色标记法
