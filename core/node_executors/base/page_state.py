...
class PageStateNodeExecutor(BaseNodeExecutor):
    ...
    def execute(self, node, context) -> Dict[str, Any]:
        ...
        context.log(
            f"[page_state] 评估页面状态: {page_name} (page_id={page_id})，特征数={len(features)}"
        )

        # 未定义任何特征时，视为不匹配，避免误判为"任意页面"
        if not features:
            context.log(
                f"[page_state] 页面 [{page_name}] 未定义任何特征，判定为不匹配", "warning"
            )
        ...
        context.log(
            f"[page_state] 页面 [{page_name}] 评估结果: {'匹配' if matched else '不匹配'} | {detail}"
        )
        ...
        if matched:
            ...
            return self.build_jump_result(
                ...
                extra={"page_id": page_id, "matched": True}
            )
    ...
    def _evaluate_features(self, features: List[Dict[str, Any]],
                           feature_mode: str, context) -> Tuple[bool, str]:
        """
        评估复合特征列表
        :return: (是否匹配, 评估明细字符串)
        """
        ...
        for idx, feature in enumerate(features):
            ...
            try:
                ...
                context.log(
                    f"[page_state] 特征 #{idx + 1} 评估异常({cond.get('type')}): {e}",
                    "error"
                )
                ok = False
    ...
    def _build_condition(feature: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        cond: Dict[str, Any] = {
            "type": feature.get("feature_type") or feature.get("type") or "image_exists"
        }
        params = feature.get("params")
        if isinstance(params, dict):
            cond.update(params)
        # 兼容：特征层级直接平铺的参数
        for key in ("template", "text", "region", "threshold",
                    "gray_scale", "gray_threshold"):
            if key in feature:
                cond.setdefault(key, feature[key])
        return cond

    @staticmethod
    def _combine_results(results: List[bool], features: List[Dict[str, Any]],
                         feature_mode: str) -> Tuple[bool, str]:
        """
        组合特征评估结果
        - 首条特征作为初始累积值
