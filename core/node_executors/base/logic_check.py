...
class LogicCheckNodeExecutor(BaseNodeExecutor):
    def _get_cond_desc(self, cond):
        """格式化 5 大类判定条件的日志描述"""
        cond_type = cond.get('condition_type', 'variable_check')
        cond_params = cond
    ...
    def execute(self, node, context):
        ...
        mode = str(params.get('logic_mode', 'and')).lower()
        timeout_ms = float(params.get('timeout', 3000))
        timeout_sec = timeout_ms / 1000.0

        if not conditions:
            context.log(' [逻辑判断] 未配置任何判定条件，默认通过', 'warning')
            return self.build_result(True)

        context.log(
            f' [逻辑判断] 开始评估条件组 (模式: {mode.upper()}, 条件数: {len(conditions)}, 超时: {int(timeout_ms)}ms)'
        )
        ...
        while True:
            ...
            time.sleep(context.get_setting('match_poll_ms', 100) / 1000.0)  # 快速轮询间隔
