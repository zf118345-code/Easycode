...
class SmartJumpNodeExecutor(BaseNodeExecutor):
    ...
    def execute(self, node, context) -> Dict[str, Any]:
        ...
        while attempt < max_retries and time.time() < deadline:
            ...
            # 计算路径
            path_result = self._compute_path(
                target_node_id, target_task_id,
                target_page_id, context
            )
            ...
            if path_result.success and path_result.path:
                ...
                context.log(
                    f"[smart_jump] 寻路成功（第 {attempt} 次）| "
                    f"路径: {' -> '.join(path_result.path)}"
                )
            ...
            context.log(
                f"[smart_jump] 第 {attempt}/{max_retries} 次寻路失败: {last_reason}",
                "warning"
            )
        ...
        # 全部重试失败
        context.log(
            f"[smart_jump] 智能跳转失败，已达最大重试次数 | 原因: {last_reason}",
            "error"
        )
        return self.build_jump_result(
            success=False,
    ...
    def _compute_path(self, target_node_id: str, target_task_id: str,
                      target_page_id: str, context) -> PathResult:
        """
        根据目标类型选择寻路方式
        ...
        if target_node_id:
            ...
            if target_page_id:
                context.log(
                    f"[smart_jump] 节点寻路失败({result.reason})，降级为页面寻路: {target_page_id}",
                    "warning"
                )
            else:
                return result
