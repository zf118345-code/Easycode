...
class BlueprintService:
    ...
    def load_blueprint(cls, project_path: str) -> dict:
        ...
        try:
            with open(bp_path, encoding='utf-8-sig') as f:
                data = json.load(f)
            ...
            raise HTTPException(status_code=500, detail=f'蓝图加载失败: {str(e)}') from e
    ...
    def save_blueprint(cls, project_path: str, data: dict):
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail=f'蓝图保存失败: {str(e2)}') from e2
        except Exception as e:
            logger.error(f'蓝图保存失败 [{bp_path}]: {e}')
            raise HTTPException(status_code=500, detail=f'蓝图保存失败: {str(e)}') from e
