...
class ExecutionDB:
    ...
    @staticmethod
    def create_execution(execution_id: str, project_path: str, task_id: str,
                         start_node_id: str = None) -> bool:
        """创建执行记录"""
    ...
    @staticmethod
    def get_status(execution_id: str) -> Optional[Dict[str, Any]]:
        """获取执行状态"""
        ExecutionDB._ensure_db()
        conn = get_connection()
        try:
    ...
    @staticmethod
    def add_log(execution_id: str, message: str, level: str = "INFO",
        ...
        try:
            ...
            # 获取当前 seq
            row = conn.execute(
                "SELECT MAX(seq) as max_seq FROM execution_logs WHERE execution_id = ?",
                (execution_id,)
            ).fetchone()
    ...
    def get_log_count(execution_id: str) -> int:
        ...
        try:
            row = conn.execute(
                "SELECT COUNT(*) as cnt FROM execution_logs WHERE execution_id = ?",
                (execution_id,)
            ).fetchone()
    ...
    def save_variable(execution_id: str, name: str, value: Any, var_type: str = None) -> bool:
        ...
        try:
            ...
            conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?)",
                (execution_id, name, json.dumps(value, ensure_ascii=False, default=str), var_type or type(value).__name__, now)
            )
            conn.commit()
            return True
        except Exception as e:
    ...
    def get_variables(execution_id: str) -> Dict[str, Any]:
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM execution_variables WHERE execution_id = ?",
                (execution_id,)
            ).fetchall()
            result = {}
            for row in rows:
    ...
    def cleanup_old_executions(max_records: int = 100):
        ...
        try:
            rows = conn.execute(
                "SELECT execution_id FROM executions ORDER BY created_at DESC LIMIT -1 OFFSET ?",
                (max_records,)
            ).fetchall()
            for row in rows:
