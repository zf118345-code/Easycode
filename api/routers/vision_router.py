...
from core.schemas import OcrTestRequestSchema, ImageTestRequestSchema
...
def create_vision_router(vision_service):
    ...
    async def create_template_folder(data: dict = Body(...)):
        ...
        return await run_in_threadpool(
            vision_service.create_template_folder, data.get("project_path"),
    ...
    async def save_region(data: dict = Body(...)):
        ...
        return await run_in_threadpool(
            vision_service.save_region, data.get("project_path"), template_name, crop_rect
        )
    ...
    async def test_ocr_recognition(request: OcrTestRequestSchema):
        if vision_service is None:
            _service_unavailable("VisionService")
        return await run_in_threadpool(
            vision_service.test_ocr, request.project_path, request.region_value,
    ...
    async def test_image_recognition(request: ImageTestRequestSchema):
        if vision_service is None:
            _service_unavailable("VisionService")
        return await run_in_threadpool(
            vision_service.test_image, request.project_path, request.template_name,
            request.gray_scale, request.gray_threshold
        )
