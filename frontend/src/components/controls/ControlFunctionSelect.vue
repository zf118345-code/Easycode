...
const viewFunction = async () => { if (!selectedFunction.value) return; await store.loadTaskData(selectedFunction.value.function_id); await store.setCanvasMode('function'); store.setFocusTarget({ type: 'graph', id: selectedFunction.value.function_id, timestamp: Date.now() }) }
</script>
<style scoped>
