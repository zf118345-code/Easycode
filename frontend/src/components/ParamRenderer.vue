    ...
    <div class="param-control">
      ...
      <template v-else-if="config.type === 'list_int4'">
        <div class="list-int4">
          <div class="coord-item"><span class="coord-label">X</span><el-input-number v-model="localValue[0]" :min="0" controls-position="right" size="small" @change="emitChange" /></div>
          <div class="coord-item"><span class="coord-label">Y</span><el-input-number v-model="localValue[1]" :min="0" controls-position="right" size="small" @change="emitChange" /></div>
          <div class="coord-item"><span class="coord-label">W</span><el-input-number v-model="localValue[2]" :min="0" controls-position="right" size="small" @change="emitChange" /></div>
      ...
      <template v-else-if="config.type === 'list_dict'">
        <div class="list-dict">
          <el-table :data="localValue" border size="small" style="width:100%">
            <el-table-column v-for="(subConfig, subKey) in config.sub" :key="subKey" :label="subConfig.label || subKey" :prop="subKey" min-width="120">
              <template #default="{ row }">
                <ParamRenderer :config="subConfig" :value="row[subKey]" @update="(val) => { row[subKey] = val; emitChange(); }" />
              </template>
            </el-table-column>
            <el-table-column label="操作" width="80">
              <template #default="{ $index }">
                <el-button type="danger" size="small" link @click="removeListItem($index)"><el-icon><Delete /></el-icon></el-button>
              </template>
            </el-table-column>
            <template #append>
              <el-button size="small" @click="addListItem"><el-icon><Plus /></el-icon> 添加</el-button>
            </template>
          </el-table>
        </div>
      </template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const getInitialValue = () => {
      ...
      if (props.config.type === 'list_int2') return Array.isArray(val) && val.length === 2 ? val : [0, 0]
    ...
    const unwatch = watch(() => props.value, (newVal) => {
      let fixedVal = newVal
      if (props.config.type === 'list_int4') fixedVal = Array.isArray(newVal) && newVal.length === 4 ? newVal : [0, 0, 0, 0]
      else if (props.config.type === 'list_int2') fixedVal = Array.isArray(newVal) && newVal.length === 2 ? newVal : [0, 0]
      else if (props.config.type === 'list_dict') fixedVal = Array.isArray(newVal) ? newVal : []
      if (JSON.stringify(fixedVal) !== JSON.stringify(localValue.value)) localValue.value = fixedVal
    }, { deep: true })
...
<style scoped>
.param-renderer { margin-bottom: 8px; }
.param-label { font-size: 13px; color: #cfd3e6; font-weight: 500; margin-bottom: 4px; }
.param-control { width: 100%; }
.file-selector { display: flex; gap: 6px; align-items: center; }
.file-selector .el-input { flex: 1; }
.list-int2, .list-int4 { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }
.coord-item { display: flex; align-items: center; gap: 4px; }
.coord-label { font-size: 12px; color: #8a8fa8; font-weight: 500; min-width: 14px; }
.coord-item .el-input-number { width: 70px; }
.dict-container { padding-left: 12px; border-left: 2px solid #3d3d5a; margin-top: 4px; }
</style>