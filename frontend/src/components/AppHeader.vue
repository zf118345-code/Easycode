<template>
  <header class="app-header">
    ...
    <div class="project-selector">
      <el-button size="small" type="primary" @click="setWorkspace">{{ workspaceStatus }}</el-button>
      <el-select v-model="store.currentProject" placeholder="选择项目" size="small" @change="onProjectChange" style="width:150px;margin-left:8px;" :disabled="!store.workspaceHandle">
        <el-option v-for="p in store.projects" :key="p" :label="p" :value="p" />
      </el-select>
    </div>
    <el-menu mode="horizontal" :default-active="activeMenu" background-color="#2d2d44" text-color="#cfd3e6" active-text-color="#409EFF" @select="onMenuSelect" class="menu-bar">
...
export default {
  ...
  methods: {
    ...
    openScreenshot() { this.$refs.screenshotTool.open() },
    async runTask() {
...
.menu-icon { color: #cfd3e6; font-size: 20px; cursor: pointer; }
.menu-icon:hover { color: #409EFF; }
.logo { color: #cfd3e6; font-weight: bold; font-size: 16px; }
.project-selector { display: flex; align-items: center; flex-shrink: 0; }
.menu-bar { flex: 1; border-bottom: none; background: transparent !important; }
.menu-bar .el-menu-item { height: 40px; line-height: 40px; }
