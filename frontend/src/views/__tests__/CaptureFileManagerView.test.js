...
describe('CaptureFileManagerView 常驻宿主协议', () => {
    ...
    it('启动只报告 ready，并可反复接收上下文而不重建页面宿主', async () => {
        ...
        await flushPromises()
        expect(captureApi.getSnapshot).toHaveBeenLastCalledWith('snap-a')
        ...
        await flushPromises()
        expect(captureApi.getSnapshot).toHaveBeenLastCalledWith('snap-b')
    ...
    it('以用户最终选择的根目录作为资源分类，而不是固定使用初始目录', async () => {
        ...
        expect(captureApi.requestAction).toHaveBeenCalled()
        wrapper.unmount()
    })
})
